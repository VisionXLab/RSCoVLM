...
def prepare_logger_for_rank(logger, rank=0):
    logger.setLevel(logging.INFO if rank == 0 else logging.WARNING)
    ch = logging.StreamHandler()
    formatter = logging.Formatter(f'%(asctime)s - %(levelname)s - %(name)s - Rank: {rank} - %(message)s', datefmt='%m/%d/%Y %H:%M:%S')
...
def partition_for_rank(all_rank_item_list: list, rank: int, world_num: int) -> list:
    ...
    elif isinstance(all_rank_item_list, GeneratorType):
        return (item for idx, item in enumerate(all_rank_item_list) if idx % world_num == rank)
    else:
...
def init_distributed_device(dist_backend="nccl", dist_url="env://", no_set_device_rank=False, timeout_min=None):
    ...
    if is_using_distributed():
        if "SLURM_PROCID" in os.environ:
            ...
            print(f'DDP via torchrun, torch.distributed.launch')
            local_rank, _, _, _ = world_info_from_env()
            torch.distributed.init_process_group(
                backend=dist_backend, init_method=dist_url, timeout=timeout
            )
...
def gather_dict(dict_to_gather, rank, world_size):
    ...
    dist.barrier()
    logging.info(f"Rank {rank} is gathering")
...
def maybe_use_hf_mirror():
    if "HTTP_PROXY" not in os.environ and "HTTPS_PROXY" not in os.environ and "ALL_PROXY" not in os.environ:
        logging.info("Using HF mirror")
        import datasets
        HF_ENDPOINT = "https://hf-mirror.com"
        os.environ["HF_ENDPOINT"] = HF_ENDPOINT
        datasets.config.HF_ENDPOINT = HF_ENDPOINT
        datasets.config.HUB_DATASETS_URL = HF_ENDPOINT + "/datasets/{repo_id}/resolve/{revision}/{path}"
    else:
        logging.info("Proxy is on, not using HF mirror")
...
def print_trainable_parameters(model):
    ...
    if is_peft_model(model):
        ...
        model_cls_name = type(model.base_model).__name__
    else:
    ...
    elif model_cls_name in ["Qwen2VLForConditionalGeneration", "Qwen2_5_VLForConditionalGeneration"]:
        named_part_modules = {
