...
def clean_string(str, ignore_casing, ignore_punctuation):
    if ignore_casing:
        str = str.lower()
...
def classification_metrics(outputs, ignore_casing=True, ignore_punctuation=True, keywords=None, **kwargs):
    ...
    for output in outputs:
        ...
        task = output['task']
        response = clean_string(response, ignore_casing, ignore_punctuation)
        ground_truth = clean_string(ground_truth, ignore_casing, ignore_punctuation)
...
def change_detection_classification(outputs, classes, skip_classes=[], height=256, width=256, ignore_casing=True, ignore_punctuation=True):
    ...
    for output in tqdm(outputs):
        predicted_class = clean_string(output['response'], ignore_casing, ignore_punctuation)
        ground_truth_class = clean_string(output["ground_truth"], ignore_casing, ignore_punctuation)
...
def compute_teochatlas_scores(data_pipe_list):
    def _compute_teochatlas_scores(dataset_name, dataset_data_pipe_list):
        outputs = []
        for item in dataset_data_pipe_list:
            ...
            input_bboxes = extract_bboxes(item["conversations"][0]['value'])
            output_bboxes = extract_bboxes(item["conversations"][1]['value'])
        ...
        if dataset_name in ["fmow_high_res", "fmow_low_res", "abcd", "cdvqa", "aid", "ucmerced", "lrben", "hrben"]:
            eval_metrics_fn = classification_metrics
        elif dataset_name in ["xbd_loc", "xbd_dmg_cls", "s2_det", "xbd_sre_qa_rqa", "s2_sre_qa", "s2_rqa", "qfabric_rqa2", "qfabric_rqa5_rtqa5", "qfabric_tre_rtqa"]:
    ...
    datasets_data_pipe_list = defaultdict(list)
    for item in data_pipe_list:
        datasets_data_pipe_list[item['dataset'].lower()].append(item)

    for dataset_name, dataset_data_pipe_list in datasets_data_pipe_list.items():
        scores[dataset_name] = _compute_teochatlas_scores(dataset_name, dataset_data_pipe_list)
