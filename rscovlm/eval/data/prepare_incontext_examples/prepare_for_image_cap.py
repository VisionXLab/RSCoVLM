...
def partition_for_rank(all_rank_item_list: list, rank: int, world_num: int) -> list:
    if isinstance(all_rank_item_list, list):
        ...
        return list(this_rank_item_list)
    else:
        raise ValueError("The input type is not supported.")
    
def incontext_max_ppl(model_path, benchmark, image_root, json_path, save_path, rank, world_size, device):
    ...
    train_filename = f'{benchmark}_train.json'

    with open(os.path.join(json_path, train_filename), 'r') as f:
    ...
    # top_k_pairs = [entry for entry in all_ppl_entries[:in_context_hot]]
    # return top_k_pairs


if __name__ == '__main__':
    model_path = "./playground/Qwen2.5-VL-7B-Instruct"
    json_path = "./playground/data/rs_caption/rs_caption_jsons/converted"
    benchmarks = ['cap_nwpu_caption', 'cap_rsicd', 'cap_rsitmd', 'cap_sydney_caption', 'cap_ucm_caption', 'vrsbench_caption']
    all_image_root = {
        'cap_nwpu_caption': "./playground/data/scene_cls/NWPU-RESISC45",
        'cap_rsicd': "./playground/data/rs_caption/rsicd/RSICD_images",
        'cap_rsitmd': "./playground/data/rs_caption/rsitmd/images",
        'cap_sydney_caption': "./playground/data/rs_caption/sydney_caption/imgs",
        'cap_ucm_caption': "./playground/data/rs_caption/ucm_caption/imgs",
        'vrsbench_caption': "./playground/data/VRSBench/Images"
    }
    ...
    for benchmark in benchmarks:
        image_root = all_image_root[benchmark]
        incontext_max_ppl(model_path, benchmark, image_root, json_path, save_path, rank, world_size, device)
