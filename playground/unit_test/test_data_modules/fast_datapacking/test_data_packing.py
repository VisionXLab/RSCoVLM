...
packing_data = False
packing_cache = "/mnt/petrelfs/liqingyun/msr/code/rscovlm/rscovlm/training/data/cache/dota_trainval512_packing-length-19800_p-random-resize-1_1epoch.jsonl"

data_args = Namespace(
    datasets=["fair1m_poly_train682"], # "rsar_poly_trainval190to1000", "dota_poly_trainval512", "dior_poly_trainval800", 
    flatten_data=True,
    data_path=None,
    image_folder=None,
...
# 统计packing mean/max/min group length
if packing_data:
    with open(packing_cache, "r") as f:
        data = [json.loads(line) for line in f if line.strip()]
    max_len, min_len, mean_len, all_data = 0, float('inf'), 0, 0
    for group in data:
        # import ipdb; ipdb.set_trace()
        max_len = max(max_len, len(group))
        min_len = min(min_len, len(group))
        all_data += len(group)
    mean_len = all_data / len(data) # data num / group num
    print(f'Group Length: max={max_len}, min={min_len}, mean={mean_len}, Group Num: {len(data)}, Data Num: {all_data}')
...
# light cache
# import ipdb; ipdb.set_trace()
# train_dataset.cache_indices(save_path="debug_tmp_packing_indices_vhm.json")

from torch.utils.data import DataLoader
train_dataloder = DataLoader(train_dataset, batch_size=batch_size, collate_fn=data_collator, num_workers=8)
