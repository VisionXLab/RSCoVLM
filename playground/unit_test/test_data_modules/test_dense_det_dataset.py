...
data_args = Namespace(
    datasets=["fair1m_train682"], # dota_poly_trainval512, 'dior_trainval800', "rsar_trainval190to1000", "fair1m_poly_train682"
    min_pixels=256 * 28 * 28, 
    max_pixels=1296 * 28 * 28,
    prob_plain_text_prompt=1,
    keep_empty_gt=True,
    # prob_random_resize=0
)
...
#     lengthes.extend(attention_mask.sum(dim=1).tolist())
#     print("max length:", max(lengthes))
# with open("./dota_trainval512_lengthes.txt", "w") as f:
...
for _ in range(10):
    ...
    lengthes.append(attention_mask.sum(dim=1))
    import ipdb; ipdb.set_trace()
    if CHECK_LOSS:
        batch_on_device = {}
...
# # 对效果差的指定模型找原因
# example_filename = 'P0438__512__1060___412.png'
# model_path = "output/qwen2-5-vl-ins_dota-poly-trainval512_tune-all_pix256-1296_lr0.00005-50epochs"
# model = Qwen2_5_VLForConditionalGeneration.from_pretrained(model_path, torch_dtype=torch.bfloat16).cuda()
# name2idx = {dd['file_name']: idx for idx, dd in enumerate(train_dataset.list_data_dict)}
# example_idx = name2idx[example_filename]
# example = train_dataset.list_data_dict[example_idx]



# 检查数据集里 `There are none.` 的比例
# num_all, num_none = 0, 0
# for i in range(len(train_dataset)):
#     message = train_dataset.get_messages(i)
#     if message[-1]['content'] == 'There are none.':
#         num_none += 1
#     num_all += 1
# print(f"num_none: {num_none}, num_all: {num_all}, ratio: {num_none / num_all:.2%}")
# import ipdb; ipdb.set_trace()
