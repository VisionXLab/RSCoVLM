...
data_args = Namespace(
    datasets=['lrsvqa_train_visual_cot'],
...
for meta in random.sample(train_dataset.list_data_dict, 50):
    ...
    w, h, wr, hr = meta['image_ori_width'], meta['image_ori_height'], meta['resized_width'], meta['resized_height']
    print(f"image size: {w}x{h} -> {wr}x{hr}")

from torch.utils.data import DataLoader
train_dataloder = DataLoader(train_dataset, batch_size=1, collate_fn=data_collator, num_workers=0, shuffle=True)
